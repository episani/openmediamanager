...
foreach ($applic_config as $config_name => $config_value){
	...
	if($config_name_exploded[0] != "DEFAULT"){
		if(isset($applic_config['DEFAULT_'.$config_nane]) && $applic_config[$config_nane]==""){
			$applic_config_default[$config_nane]=$applic_config['DEFAULT_'.$config_nane];
		} else {
			$applic_config_default[$config_nane]=$applic_config[$config_nane];
		}
	}
}
...
if($applic_config['WLAN0_VPN_ADDR'] !="" &&
	...
	$applic_config['WLAN0_VPN_DHCP_NETMASK'] !="" &&
	...
	$applic_config_marker['###WLAN0_VPN_DHCP_NETMASK###']=$applic_config['WLAN0_VPN_DHCP_NETMASK'];
